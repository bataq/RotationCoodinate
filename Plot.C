...
{   
  ...
  TCut ecut_M=Form("ph_merge<=%d",energy_M);
  TCut type_select = "stype == 10||stype == 11||stype == 20||stype == 21||stype == 22";
  // projection region
  ...
  tree_subpix->Draw("yy:xx>>subpixmap",ecut_m && ecut_M && type_select,"colz");
  
  canvas->cd(2);
  TH2D *reg_map = new TH2D("regionmap","Projection Region; CA; RA",rM,0,rM,rM,0,rM); 
  tree_subpix->Draw("yy:xx>>regionmap",xxm && xxM && yym && yyM,"scat");
  ...
  *pro_map = *rot_map;
  ...
  std::cout<<Form(" yy<=%d",p_yrM)<<std::endl;
  std::cout<<" stype==single && double"<<std::endl;
  std::cout<<"########################"<<std::endl;
...
void WriteConditions(TCut ecut_m,TCut ecut_M,TCut xxm,TCut xxM,TCut yym,TCut yyM){
  ...
  TText *t7=new TText(0.1,0.2,"stype==single&&double");
